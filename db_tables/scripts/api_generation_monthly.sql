...
WITH region_demand_rank as(
    ...
    FROM combined_gen
    GROUP BY country_or_region, country_code, generation_date, fossil_flag
), latest_year as(
    SELECT country_or_region, MAX(year(generation_date)) as max_year 
    FROM expanded_table
    WHERE year(generation_date) <= {api_year}
    GROUP BY country_or_region
)
SELECT
